...
powershell -NonInteractive -NoProfile -ExecutionPolicy Bypass -Command "$batt=Get-CimInstance Win32_Battery -ErrorAction SilentlyContinue; if($batt){$status=@{1='Discharging';2='AC Power (Charging)';3='Fully Charged';4='Low';5='Critical';6='Charging+High';7='Charging+Low';8='Charging+Critical';9='Unknown';10='Partially Charged'}; Write-Host '    Battery Status:' ($status[$batt.BatteryStatus]??'Unknown'); Write-Host '    Charge Level:' $batt.EstimatedChargeRemaining'%'; Write-Host '    Time Remaining:' $batt.EstimatedRunTime 'min'}else{Write-Host '    Battery info not available — charger connected or battery issue'}"
echo.
...
echo    Battery degraded hai → replacement ticket
